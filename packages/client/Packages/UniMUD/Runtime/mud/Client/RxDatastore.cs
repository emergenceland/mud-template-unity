...
namespace mud
{
    ...
    public class RxDatastore
    {
        ...
        public void RegisterTable(RxTable table, string? name = null)
        {
            ...
            Debug.Log("Registered table: " + (name ?? table.Id));
        ...
        public RxRecord? GetValue(RxTable table, string key) { return GetValue(table.Id, key);}
        public RxRecord? GetValue(string table, string key)
        {
            var hasTable = store.ContainsKey(table);
            if (!hasTable) return null;
            var hasKey = store[table].Values.TryGetValue(key, out var value);
